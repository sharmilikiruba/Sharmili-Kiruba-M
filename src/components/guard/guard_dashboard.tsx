...
export default function Dashboard() {
  ...
  useEffect(() => {
    const fetchDashboardData = async () => {
      try {
        setIsLoading(true);
        const res = await apiClient.get('/guard/dashboard');
        if (res.data.success) {
          const { guard, stats: dashboardStats, recentEntries: entries } = res.data.data;

          setGuardName(guard.name);
          setGuardDetails(`${guard.gate} • ${guard.shift}`);

          setStats([
            { label: 'Active Visitors', value: dashboardStats.activeVisitors.toString(), icon: Users },
            { label: 'Entries Today', value: dashboardStats.entriesToday.toString(), icon: LogIn },
            { label: 'Exits Today', value: dashboardStats.exitsToday.toString(), icon: LogOut },
            { label: 'Pending Exits', value: dashboardStats.pendingExits.toString(), icon: UserCheck },
          ]);
