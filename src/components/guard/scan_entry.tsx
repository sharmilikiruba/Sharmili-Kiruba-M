...
export default function ScanEntry() {
  ...
  const handleScanAction = async () => {
    ...
    try {
      const res = await apiClient.post('/guard/scan', { qrData: qrCode });
      if (res.data.success) {
        const data = res.data.data;
        // Check if visitor is already inside (based on status or active logs)
        // Assuming backend returns current status or active log details
        ...
        setVisitorData({
          ...
          entryTime: data.activeLog ? new Date(data.activeLog.entryTime).toLocaleString() : '',
          durationInside: data.activeLog ? 'Calculating...' : '', // You could calculate diff here
          isOverdue: false, // Calculate based on time
          photoUrl: data.photoUrl
        });

        // Determine mode based on whether there is an active log without exit time
        if (data.activeLog && !data.activeLog.exitTime) {
          setVisitorMode('INSIDE');
        } else {
  ...
  const handleFinalize = async (action: 'entry' | 'exit' | 'deny') => {
    ...
    try {
      ...
      if (res.data.success) {
        alert(`${action === 'entry' ? 'Entry' : 'Exit'} confirmed successfully`);
        router.push('/Guard/guard_dashboard');
      }
    } catch (err: any) {
      alert(err.response?.data?.message || `Failed to confirm ${action}`);
    } finally {
      setIsLoading(false);
    }
  };
